  ...
  if (timelineButtons.length > 0 && timelineItems.length > 0) {
    ...
    if (prevButton && nextButton) {
      prevButton.addEventListener("click", () => {
        currentIndex =
          currentIndex === 0 ? timelineButtons.length - 1 : currentIndex - 1;
        timelineButtons[currentIndex].click();
      });

      nextButton.addEventListener("click", () => {
        currentIndex =
          currentIndex === timelineButtons.length - 1 ? 0 : currentIndex + 1;
        timelineButtons[currentIndex].click();
      });
    }
...
function setupProjetos() {
  ...
  if (titulosProjetos.length > 0 && projetos.length > 0 && containerProjetos) {
    titulosProjetos.forEach((titulo) => {
      titulo.addEventListener("mouseenter", () => {
        ...
        if (projetoAtivo) {
          ...
          containerProjetos.style.height =
            projetoAtivo.scrollHeight + 50 + "px";
        } else {
    ...
    window.addEventListener("resize", () => {
      const projetoAtivo = containerProjetos.querySelector(
        ".projeto-container.ativo"
      );
      if (projetoAtivo) {
...
(function () {
  ...
  const divRasgos = document.querySelectorAll(".img__rasgo");
  ...
  if (btnContraste) {
    btnContraste.addEventListener("click", () => {
      ...
      if (!modoContraste) {
        for (let i = 0; i < divRasgos.length; i++) {
          divRasgos[i].style.display = "none";
        }
        modoContraste = true;
      } else {
        for (let i = 0; i < divRasgos.length; i++) {
          divRasgos[i].style.display = "flex";
        }
        modoContraste = false;
      }
    });
